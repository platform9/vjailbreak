...
2. **Browse (US2)**: On the Templates tab, search by the name you just used, filter by "Private" (default visibility), sort by "Last used" — confirm the card you saved is findable and its tags/summary match what you entered.
...
5. **Detail view (US4)**: Click a card (not "Use") — confirm the `DrawerShell`-based detail drawer opens over a dimmed list, shows Owner/Times Used/Last Used/Created plus full Source & Destination and Network & Storage Mappings sections, and closes cleanly via the X or backdrop click.
