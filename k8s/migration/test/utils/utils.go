...
func warnError(err error) {
	if _, err := fmt.Fprintf(GinkgoWriter, "warning: %v\n", err); err != nil {
		log.Printf("Failed to write to GinkgoWriter: %v", err)
	}
}
...
func Run(cmd *exec.Cmd) ([]byte, error) {
	...
	if err != nil {
		return output, fmt.Errorf("%s failed with error: (%v) %s", command, err, strings.TrimSpace(string(output)))
	}
...
// LoadImageToKindClusterWithName loads a local docker image to a kind cluster with the specified name
