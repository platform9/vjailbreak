...
# Step 2: Check for partitions with bootable flag set
...
# Step 3: Try to find which disk's MBR contains GRUB signature
...
# Step 4: Check for /boot partition in fstab
if [ -z "$bootdisk" ]; then
...
# Step 5: Check for LVM root device
if [ -z "$bootdisk" ]; then
