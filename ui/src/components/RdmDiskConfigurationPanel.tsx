...
export const RdmDiskConfigurationPanel: React.FC<
  RdmDiskConfigurationPanelProps
> = ({ rdmDisks, openstackCreds, selectedVMs, onConfigurationChange }) => {
  ...
  useEffect(() => {
    if (rdmDisks.length > 0 && !initializedRef.current) {
      const initialConfigs = rdmDisks.map((disk) => ({
        ...
        cinderBackendPool:
          disk.spec.openstackVolumeRef?.cinderBackendPool || "",
  ...
  const availableBackendPools =
    openstackCreds?.status?.openstack?.volumeBackends || [];
  const availableVolumeTypes =
    openstackCreds?.status?.openstack?.volumeTypes || [];
    ...
    <Box sx={{ mt: 3 }}>
      ...
      <Alert severity="info" sx={{ mb: 2 }}>
        <Typography variant="body2">
          Configure OpenStack Cinder settings for each RDM disk. These settings
          will be applied when migrating the selected VMs.
        </Typography>
      </Alert>
          ...
          <Card key={disk.metadata.name} sx={{ mb: 2 }}>
            <CardContent>
                ...
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      label="Volume Type *"
                      onChange={(e) =>
                        updateConfiguration(index, "volumeType", e.target.value)
                      }
                    >
              ...
              <Box>
                ...
                <Alert severity="info" sx={{ mb: 2 }}>
                  <Typography variant="body2">
                    <strong>Hint:</strong> Edit this field in the VMware Notes
                    section.
                    <br />
                ...
                {Object.entries(config.source).map(
