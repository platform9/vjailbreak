...
export function useApplyTemplatePrefill({
  ...
  useEffect(() => {
    ...
    updateParams({
      ...
      cutoverStartTime: templatePrefill.cutoverStartTime,
      cutoverEndTime: templatePrefill.cutoverEndTime,
    ...
    updateSelectedOptions({
      ...
      // Without these the restored times sit in params but their fields stay collapsed,
      // so a time-window cutover looks empty.
      cutoverStartTime: Boolean(templatePrefill.cutoverStartTime),
      cutoverEndTime: Boolean(templatePrefill.cutoverEndTime),
      useGPU: templatePrefill.useGPU || false,
