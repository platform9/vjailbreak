...
const CustomToolbar = ({ onRefresh, onAddBMConfig }: CustomToolbarProps) => {
  return (
    <GridToolbarContainer
      sx={{
        p: 2,
...
export default function BMConfigTable({ onAddBMConfig }) {
  ...
  const rows = bmConfigs
    ? bmConfigs.map((config) => ({
  ...
  return (
    <div style={{ height: 'calc(100vh - 180px)', width: '100%', overflow: 'hidden' }}>
      <DataGrid
        ...
        slots={{
          toolbar: () => <CustomToolbar onRefresh={handleRefresh} onAddBMConfig={onAddBMConfig} />,
