    ...
    renderCell: (params) => {
      ...
      return (
        <Tooltip
          title={
            isDisabled
              ? 'Cannot delete while conversion is in progress'
              : 'Delete cluster conversion'
          }
        >
          <IconButton
            onClick={(e) => {
...
interface CustomToolbarProps {
  ...
  ) => Promise<QueryObserverResult<ClusterMigration[], Error>>
}
...
export default function ClusterMigrationsTable({
  ...
  const clusterMigrationsWithActions =
    clusterMigrations?.map((clusterMigration) => ({
      ...clusterMigration,
      ...
      slots={{
        toolbar: () => (
          <CustomToolbar
            ...
            onDeleteSelected={() => {
              const selectedClusterMigrations = clusterMigrations?.filter((m) =>
