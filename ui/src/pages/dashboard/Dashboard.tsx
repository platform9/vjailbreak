...
export default function Dashboard() {
  ...
  const [deleteType, setDeleteType] = useState<'migration' | 'clusterMigration' | 'esxiMigration'>(
    'migration'
  )
  ...
  const handleDeleteClusterMigration = async (clusterMigrations: ClusterMigration[]) => {
    await Promise.all(
      clusterMigrations.map((migration) => deleteClusterMigration(migration.metadata.name))
    )
  ...
  const handleDeleteESXIMigration = async (esxiMigrations: ESXIMigration[]) => {
    await Promise.all(
      esxiMigrations.map((migration) => deleteESXIMigration(migration.metadata.name))
    )
  ...
  const handleTabChange = (_event: React.SyntheticEvent, newValue: number) => {
    ...
    const tabNames = ['migrations', 'agents', 'credentials', 'clusterconversions', 'maasconfig']
