...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "  --timestamp         Use timestamp prefix (YYYYMMDD-HHMMSS) instead of configured numbering"
...
# Determine configured branch numbering mode from extension config or legacy init options.
# Supported values: sequential, timestamp, descriptive
get_branch_numbering_mode() {
    local cfg_file="$REPO_ROOT/.specify/extensions/git/git-config.yml"
    local legacy_file="$REPO_ROOT/.specify/init-options.json"
    local mode=""

    if [ -f "$cfg_file" ]; then
        mode=$(grep -E '^[[:space:]]*branch_numbering:' "$cfg_file" | head -1 | cut -d':' -f2- | sed -E 's/^[[:space:]]*//; s/[[:space:]]*$//' | sed 's/^['"'"']//' | sed 's/['"'"']$//')
    fi

    if [ -z "$mode" ] && [ -f "$legacy_file" ]; then
        mode=$(grep -E '"branch_numbering"' "$legacy_file" | head -1 | sed -E 's/.*:[[:space:]]*"([^"]+)".*/\1/')
    fi

    case "$mode" in
        sequential|timestamp|descriptive)
            printf '%s' "$mode"
            ;;
        *)
            printf 'sequential'
...
if [ -n "${GIT_BRANCH_NAME:-}" ]; then
    ...
    BRANCH_NUMBERING_MODE=$(get_branch_numbering_mode)
    if [ "$USE_TIMESTAMP" = true ]; then
        FEATURE_NUM=$(date +%Y%m%d-%H%M%S)
        BRANCH_NAME="${FEATURE_NUM}-${BRANCH_SUFFIX}"
    elif [ "$BRANCH_NUMBERING_MODE" = "descriptive" ]; then
...
if [ -n "${GIT_BRANCH_NAME:-}" ] && [ "$BRANCH_BYTE_LEN" -gt $MAX_BRANCH_LENGTH ]; then
    ...
    if [ "$BRANCH_NUMBERING_MODE" = "descriptive" ]; then
        TRUNCATED_SUFFIX=$(echo "$BRANCH_NAME" | cut -c1-$MAX_BRANCH_LENGTH)
        TRUNCATED_SUFFIX=$(echo "$TRUNCATED_SUFFIX" | sed 's/-$//')

        ORIGINAL_BRANCH_NAME="$BRANCH_NAME"
        BRANCH_NAME="$TRUNCATED_SUFFIX"
    else
