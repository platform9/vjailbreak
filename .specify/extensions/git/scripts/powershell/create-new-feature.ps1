...
if ($Help) {
    ...
    Write-Host "  -Timestamp          Use timestamp prefix (YYYYMMDD-HHMMSS) instead of configured numbering"
...
function Get-BranchNumberingMode {
    $configFile = Join-Path $repoRoot '.specify/extensions/git/git-config.yml'
    $legacyFile = Join-Path $repoRoot '.specify/init-options.json'
    $mode = ''

    if (Test-Path $configFile) {
        $line = Get-Content $configFile | Select-String '^[\s]*branch_numbering:' | Select-Object -First 1
        if ($line -and $line.Line -match '^[\s]*branch_numbering:\s*["'']?([^"'']+)["'']?') {
            $mode = $matches[1]
        }
    }

    if ([string]::IsNullOrWhiteSpace($mode) -and (Test-Path $legacyFile)) {
        $line = Get-Content $legacyFile | Select-String '"branch_numbering"' | Select-Object -First 1
    ...
    switch ($mode.ToLower()) {
        ...
        'descriptive' { return 'descriptive' }
...
if ($env:GIT_BRANCH_NAME) {
    ...
    $branchNumberingMode = Get-BranchNumberingMode
    if ($Timestamp) {
        $featureNum = Get-Date -Format 'yyyyMMdd-HHmmss'
        $branchName = "$featureNum-$branchSuffix"
    } elseif ($branchNumberingMode -eq 'descriptive') {
...
if ($branchName.Length -gt $maxBranchLength) {
    if ($branchNumberingMode -eq 'descriptive') {
        $truncatedBranchName = $branchName.Substring(0, [Math]::Min($branchName.Length, $maxBranchLength))
        $truncatedBranchName = $truncatedBranchName -replace '-$', ''

        $originalBranchName = $branchName
        $branchName = $truncatedBranchName
    } else {
