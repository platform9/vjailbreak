...
check_feature_branch() {
    ...
    if [[ "$is_sequential" != "true" ]] && [[ ! "$branch" =~ ^[0-9]{8}-[0-9]{6}- ]] && [[ ! "$branch" =~ ^[A-Za-z0-9]+(-[A-Za-z0-9]+)*$ ]]; then
        echo "ERROR: Not on a feature branch. Current branch: $raw" >&2
        echo "Feature branches should be named like: add-feature-name, 001-feature-name, or 20260319-143022-feature-name" >&2
        return 1
    fi
