...
vJailbreak works by running itself a a VM on the target OpenStack cloud. It connects remotely
...
Download and install [ORAS](https://oras.land/docs/installation). A toolkit to dowload the qcow2 image of vJailbreak. Then, download the latest version of the vJailbreak image with the following command:
  ```shell
...
- Use larger flavor or larger VM migration
