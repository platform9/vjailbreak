...
In environments where IP addresses are assigned manually after VM deployment (L2-only networks without DHCP), the vJailbreak installation script waits until:
- A non-loopback IPv4 address is assigned to the VM
- A default route is configured

The script logs its status to `/var/log/pf9-install.log`. Once you assign an IP and configure routing, installation proceeds automatically. No manual restart is required.

```bash
# Example: Assign static IP and route
