...
Download and install [ORAS](https://oras.land/docs/installation), a toolkit to download the qcow2 image of vJailbreak. Then, download the latest version of the vJailbreak image with the following command:
  ...
  oras pull quay.io/platform9/vjailbreak:v0.3.0
  ```
...
- Reserve IP for VM ( In PCD Go to Physical Networks > Select network > Go to Port Tab > Create Port by reserving IP Address.)
...
- Recommended to Create VM by specifying From Volume instead of image and volume size should be - 160 GB
