...
func (osclient *OpenStackClients) CheckIfPortExists(ipEntries []vm.IpEntry, mac string, network *networks.Network, gatewayIP map[string]string) (*ports.Port, error) {
...
func (osclient *OpenStackClients) GetCreateOpts(network *networks.Network, mac string, ipEntries []vm.IpEntry, vmname string, securityGroups []string, gatewayIP map[string]string) (ports.CreateOpts, error) {
	...
	if len(ipEntries) > 0 {
		...
		subnetID, err := subnets.Get(osclient.NetworkingClient, network.Subnets[0]).Extract()
		if err != nil {
...
func (osclient *OpenStackClients) ValidateAndCreatePort(network *networks.Network, mac string, ipPerMac map[string][]vm.IpEntry, vmname string, securityGroups []string, fallbackToDHCP bool, gatewayIP map[string]string) (*ports.Port, error) {
	...
	Existingport, err := osclient.CheckIfPortExists(ipPerMac[mac], mac, network, gatewayIP)
	if err != nil {
		return nil, err
	}
	if Existingport != nil {
	...
	createOpts, err := osclient.GetCreateOpts(network, mac, ipPerMac[mac], vmname, securityGroups, gatewayIP)
	if err != nil {
		if !fallbackToDHCP {
			...
			return osclient.CreatePortWithDHCP(network, ipPerMac, mac, gatewayIP, createOpts)
		}
	}
	return osclient.CreatePort(createOpts)
}

func (osclient *OpenStackClients) CreatePortWithDHCP(network *networks.Network, ipPerMac map[string][]vm.IpEntry, mac string, gatewayIP map[string]string, createOpts ports.CreateOpts) (*ports.Port, error) {

	dhcpPort, dhcpErr := osclient.CreatePort(createOpts)
...
func (osclient *OpenStackClients) CreatePort(createOpts ports.CreateOpts) (*ports.Port, error) {
	return ports.Create(osclient.NetworkingClient, createOpts).Extract()
}
