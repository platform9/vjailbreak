...
func (osclient *OpenStackClients) CreateVM(flavor *flavors.Flavor, networkIDs, portIDs []string, vminfo vm.VMInfo, availabilityZone string, securityGroups []string, vjailbreakSettings k8sutils.VjailbreakSettings, useFlavorless bool) (*servers.Server, error) {
	...
	// Get all IPs for each port to include in cloud-init
	portIPs := make(map[string][]string) // portID -> []ips
	for _, portID := range portIDs {
		port, err := osclient.GetPort(portID)
		if err != nil {
			return nil, fmt.Errorf("failed to get port %s: %v", portID, err)
		}
		for _, fixedIP := range port.FixedIPs {
			portIPs[portID] = append(portIPs[portID], fixedIP.IPAddress)
		}
		// For the first IP, OpenStack will handle it, we need to handle additional IPs
		if len(portIPs[portID]) > 1 {
			PrintLog(fmt.Sprintf("Port %s has multiple IPs: %v", portID, portIPs[portID]))
		}
	}

	// Prepare cloud-init network configuration for additional IPs
	if len(portIPs) > 0 {
		var networkConfig []map[string]interface{}
		
		for portID, ips := range portIPs {
			if len(ips) <= 1 {
				continue // Skip if only one IP (handled by OpenStack)
			}
			...
			network, err := osclient.GetNetwork(port.NetworkID)
			if err != nil {
				return nil, fmt.Errorf("failed to get network %s: %v", port.NetworkID, err)
			}

			// Create network configuration for this interface
			ifConfig := map[string]interface{}{
				"type": "physical",
				"name": fmt.Sprintf("eth%d", len(networkConfig)),
				"mac_address": mac,
				"subnets": []map[string]interface{}{},
			}

			// Add primary IP (already handled by OpenStack, but we include it for completeness)
			if len(ips) > 0 {
				subnet, err := osclient.GetSubnet(network.Subnets, ips[0])
				if err == nil {
					...
					if len(port.AllowedAddressPairs) > 0 {
						primarySubnet["gateway"] = port.AllowedAddressPairs[0].IPAddress
					}
					ifConfig["subnets"] = append(ifConfig["subnets"].([]map[string]interface{}), primarySubnet)
				}
			}

			// Add additional IPs as alias interfaces
			for i := 1; i < len(ips); i++ {
				subnet, err := osclient.GetSubnet(network.Subnets, ips[i])
				if err != nil {
					PrintLog(fmt.Sprintf("Warning: Could not find subnet for IP %s: %v", ips[i], err))
					continue
				}

				aliasSubnet := map[string]interface{}{
					"type":    "static",
					"address": ips[i],
					"netmask": subnet.CIDR,
				}
				if len(port.AllowedAddressPairs) > 0 {
					aliasSubnet["gateway"] = port.AllowedAddressPairs[0].IPAddress
				}

				aliasConfig := map[string]interface{}{
					"type":    "physical",
					"name":    fmt.Sprintf("eth%d:%d", len(networkConfig), i-1),
					"mac_address": mac,
					"subnets": []map[string]interface{}{aliasSubnet},
				}

				networkConfig = append(networkConfig, aliasConfig)
			}

			networkConfig = append(networkConfig, ifConfig)
		}

		// Convert network config to cloud-init format
		if len(networkConfig) > 0 {
			...
			// Convert network config to cloud-init v2 format
			for _, iface := range networkConfig {
				ethName := iface["name"].(string)
				cloudConfig["network"].(map[string]interface{})["ethernets"].(map[string]interface{})[ethName] = map[string]interface{}{
					"match": map[string]string{
						"macaddress": iface["mac_address"].(string),
					},
					"addresses": []string{},
				}

				// Add all IPs to the interface
				for _, subnet := range iface["subnets"].([]map[string]interface{}) {
					if ip, ok := subnet["address"].(string); ok {
						cidr := subnet["netmask"].(string)
						if !strings.Contains(cidr, "/") {
							// Convert netmask to CIDR if needed
							cidr = fmt.Sprintf("%s/%s", ip, cidr)
						}
						cloudConfig["network"].(map[string]interface{})["ethernets"].(map[string]interface{})[ethName].(map[string]interface{})["addresses"] = 
							append(cloudConfig["network"].(map[string]interface{})["ethernets"].(map[string]interface{})[ethName].(map[string]interface{})["addresses"].([]string), cidr)
					}

					// Add gateway if present
					if gw, ok := subnet["gateway"].(string); ok && gw != "" {
						cloudConfig["network"].(map[string]interface{})["ethernets"].(map[string]interface{})[ethName].(map[string]interface{})["gateway4"] = gw
					}
				}
			}
			...
			if err != nil {
				return nil, fmt.Errorf("failed to marshal cloud-init network config: %v", err)
			}

			// Add to server creation options
			serverCreateOpts.UserData = yamlData
		}
	}
