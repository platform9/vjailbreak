...
func (osclient *OpenStackClients) CreatePort(network *networks.Network, mac, ip, vmname string, securityGroups []string) (*ports.Port, error) {
	...
	if err != nil {
		...
		PrintLog(fmt.Sprintf("Could Not Use IP: %s, using DHCP to create Port Error: %s", ip, err))
...
func (osclient *OpenStackClients) CreateVM(flavor *flavors.Flavor, networkIDs, portIDs []string, vminfo vm.VMInfo, availabilityZone string, securityGroups []string, vjailbreakSettings VjailbreakSettings, useFlavorless bool) (*servers.Server, error) {
	uuid := ""
	bootableDiskIndex := 0
