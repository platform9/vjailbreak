		...
		if incrementalCopyCount == 0 {
			...
			for idx := range vminfo.VMDisks {
				...
				if len(changedAreas.ChangedArea) == 0 {
					...
					startTime := time.Now()
					migobj.logMessage(fmt.Sprintf("Starting incremental block copy for disk %d", idx))
					...
					migobj.logMessage(fmt.Sprintf("Finished copying changed blocks for disk %d in %s", idx, duration))
