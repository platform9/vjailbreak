...
func (migobj *Migrate) ReservePortsForVM(vminfo *vm.VMInfo) ([]string, []string, []string, error) {
	...
	if len(migobj.Networkports) != 0 {
		...
		for idx, networkname := range networknames {
			...
			port, err := openstackops.ValidateAndCreatePort(network, vminfo.Mac[idx], vminfo.IPperMac, vminfo.Name, securityGroupIDs, migobj.FallbackToDHCP, vminfo.GatewayIP)
			if err != nil {
