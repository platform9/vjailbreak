...
func (migobj *Migrate) CreateTargetInstance(vminfo vm.VMInfo) error {
	...
	if migobj.UseFlavorless {
		if migobj.TargetFlavorId == "" {
			err = fmt.Errorf("flavorless creation is enabled, but TargetFlavorId in vmwaremachine %s is empty. Please set it to the ID of your base flavor (e.g., '0-0-x')", mogobj.VmwaremachineName)
