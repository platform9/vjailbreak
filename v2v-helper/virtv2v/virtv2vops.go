...
func ReinstallGrubLegacy(disks []vm.VMDisk) error {
	...
	// Build the exact list of guest disk names inside the guestfish appliance.
	// guestfish (QEMU direct backend) presents the N disks passed via -a as
	// /dev/sda … /dev/sd{a+N-1} in the same order they were supplied.
	// We generate EXACTLY N names — no more — so we never accidentally include
	// the guestfish appliance's own scratch disk (which appears as /dev/sd{a+N}
	// when there are N guest disks, i.e. /dev/sde for a 4-disk guest).
	sdNames := make([]string, len(disks))
	vdNames := make([]string, len(disks))
	for i := range disks {
		sdNames[i] = fmt.Sprintf("/dev/sd%c", rune('a'+i))
		vdNames[i] = fmt.Sprintf("/dev/vd%c", rune('a'+i))
	}
	exactSDList := strings.Join(sdNames, " ")
	exactVDList := strings.Join(vdNames, " ")

	log.Printf("ReinstallGrubLegacy: exact guest disk list (sd): %s", exactSDList)
	log.Printf("ReinstallGrubLegacy: exact guest disk list (vd): %s", exactVDList)
	...
	//
	// %s placeholders (in order): exactSDList, exactVDList
	grubReinstallScript := fmt.Sprintf(`#!/bin/sh
...
#
# 3. We use grub's "find <GRUB_DIR>/stage2" to determine the (hdN,P) that holds
#    the GRUB stage directory.  This avoids all df / arithmetic and lets grub
#    be authoritative.
#
# 4. Only the exact N guest disks are enumerated (no appliance scratch disks).
...
# ── Show current device.map and menu.lst ────────────────────────────────────
echo "vjailbreak: current device.map:"
cat "$GRUB_DIR/device.map" 2>/dev/null || echo "(not found)"
echo "vjailbreak: current menu.lst:"
cat "$GRUB_DIR/menu.lst" 2>/dev/null || echo "(not found)"

# ── Enumerate EXACTLY the N guest block devices ─────────────────────────────
# The list below was generated by the Go migration code from len(vminfo.VMDisks).
# It intentionally contains only the guest's own disks — never the guestfish
# appliance scratch disk (which would appear as /dev/sd{a+N} for N guest disks).
DISKS=$(ls %s 2>/dev/null | sort) || true
if [ -z "$DISKS" ]; then
...
echo "vjailbreak: guest disks in guestfish appliance: $DISKS"
...
# "find <GRUB_DIR>/stage2" scans all accessible (hdN,P) and returns the
# partition containing the file.  This is the authoritative, arithmetic-free
# way to determine the GRUB root.
GRUB_STAGE2_PATH="${GRUB_DIR}/stage2"
FIND_CMDS="${DEVICE_CMDS}find ${GRUB_STAGE2_PATH}
quit"
echo "vjailbreak: running grub find ${GRUB_STAGE2_PATH}..."
...
if [ -z "$GRUB_ROOT" ]; then
    echo "vjailbreak: ERROR - grub could not locate ${GRUB_STAGE2_PATH} on any disk"
    exit 1
fi
...
GRUB_RC=0
SETUP_OUT=$(printf '%%s' "$SETUP_CMDS" | "$GRUB_BIN" --no-curses --batch 2>&1) || GRUB_RC=$?
echo "vjailbreak: grub setup output:"
echo "$SETUP_OUT"
...
# ── Verify and fix menu.lst root entries ─────────────────────────────────────
# virt-v2v may have written an incorrect "root (hdX,Y)" line in menu.lst.
# The GRUB_ROOT value from "grub find" above is authoritative: it is the
# (hdN,P) tuple that actually contains /boot/grub/stage2.
# We patch every "root (hdX,Y)" line in menu.lst to use GRUB_ROOT so that
# GRUB stage2's first command ("root (hdN,P)") cannot fail with Error 21.
# NOTE: We match only the grub "root" command (space before paren), NOT the
# kernel "root=/dev/..." boot parameter (equals sign, no space+paren).
MENU_LST="$GRUB_DIR/menu.lst"
if [ -f "$MENU_LST" ]; then
    MENU_ROOT=$(grep -o 'root (hd[0-9]*,[0-9]*)' "$MENU_LST" 2>/dev/null | head -1 | sed 's/root //')
    if [ -n "$MENU_ROOT" ] && [ "$MENU_ROOT" != "$GRUB_ROOT" ]; then
        echo "vjailbreak: WARNING - menu.lst root is $MENU_ROOT but grub find says $GRUB_ROOT - fixing"
        sed -i "s|root (hd[0-9]*,[0-9]*)|root $GRUB_ROOT|g" "$MENU_LST"
        echo "vjailbreak: menu.lst after root fix:"
        cat "$MENU_LST"
    else
        echo "vjailbreak: menu.lst root is correct: $GRUB_ROOT"
    fi
else
    echo "vjailbreak: menu.lst not found at $MENU_LST - skipping root verification"
fi

echo "vjailbreak: GRUB reinstall complete"
`, exactSDList, exactVDList)
