...
build {
  ...
  provisioner "file" {
    source      = "${path.root}/scripts/chhtpasswd.sh"
    destination = "/tmp/chhtpasswd.sh"
  }
  provisioner "file" {
  ...
  provisioner "shell" {
    ...
    "sudo mv /tmp/chhtpasswd.sh /etc/pf9/chhtpasswd.sh",
    ...
    "sudo chmod +rwx /etc/pf9/chhtpasswd.sh",
